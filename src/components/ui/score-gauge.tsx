...
export interface ScoreGaugeProps extends React.HTMLAttributes<HTMLDivElement> {
  /** Score from 0-100. Alias: use `value` or `score`. */
  score?: number;
  /** Score from 0-100 (alias for score) */
  value?: number;
  size?: keyof typeof gaugeSizes | number;
  strokeWidth?: number;
  label?: string;
}
...
const ScoreGauge = React.forwardRef<HTMLDivElement, ScoreGaugeProps>(
  ({ className, score: scoreProp, value: valueProp, size: sizeProp = 200, strokeWidth = 14, label, ...props }, ref) => {
    const clampedScore = Math.min(100, Math.max(0, scoreProp ?? valueProp ?? 0));
    const color = getScoreColor(clampedScore);

    const numericSize = typeof sizeProp === 'string' ? gaugeSizes[sizeProp] : sizeProp;
    ...
    return (
      ...
      >
        <svg
          width={numericSize}
          height={numericSize / 2 + strokeWidth}
          viewBox={`0 0 ${numericSize} ${numericSize / 2 + strokeWidth}`}
          fill="none"
        ...
        <div
          ...
          style={{ top: numericSize / 2 - 10, left: '50%', transform: 'translateX(-50%)' }}
        >
          <span
