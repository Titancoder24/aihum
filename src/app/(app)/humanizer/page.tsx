...
export default function HumanizerPage() {
  ...
  const handleHumanizeAgain = useCallback(async () => {
    if (!result?.humanized || isLoading) return;
    await humanize(result.humanized, mode);
            ...
            <div className="grid gap-6 sm:grid-cols-2">
              <Card className="border-white/[0.06] bg-[#141420]">
                <CardContent className="flex flex-col items-center py-8">
                  ...
                  <ScoreGauge value={result.beforeScore} size="md" />
                  <p className="mt-3 text-sm text-white/50">
                    AI Score: {Math.round(result.beforeScore)}%
                  </p>
                </CardContent>
              </Card>
              <Card className="border-white/[0.06] bg-[#141420]">
                <CardContent className="flex flex-col items-center py-8">
                  ...
                  <ScoreGauge value={result.afterScore} size="md" />
                  <p className="mt-3 text-sm text-white/50">
                    AI Score: {Math.round(result.afterScore)}%
                  </p>
                </CardContent>
              </Card>
            </div>
